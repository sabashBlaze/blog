...
function get_search_form( $echo = true ) {
	...
	if ( '' != $search_form_template ) {
		...
		if ( 'html5' == $format ) {
			...
			$form = '<form role="search" method="get" id="blog_searchform" class="searchform" action="' . esc_url( home_url( '/' ) ) . '">
				<div>
					<label class="screen-reader-text" for="s">' . _x( 'Search for:', 'label' ) . '</label>
					<div class="input-append">
					<input class="span2" type="text" value="' . get_search_query() . '" name="s" id="s" />
					<input class="btn" type="submit" id="searchsubmit" value="'. esc_attr_x( 'Search', 'submit button' ) .'" />
					</div>
				</div>
			</form>';
		}
